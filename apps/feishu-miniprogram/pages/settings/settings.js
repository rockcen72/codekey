...
Page({
    ...
    onShow() {
        const paired = (0, storage_1.hasAuth)();
        this.setData({
            isPaired: paired,
            deviceId: (0, storage_1.getDeviceId)() || '',
            hasE2EKey: !!(0, storage_1.getContentKey)(),
            e2eStatus: (0, storage_1.getE2EStatus)(),
        });
        if (paired) {
        ...
        tt.scanCode({
            ...
            success: (res) => {
                ...
                if (code) {
                    ...
                    if (keyId)
                        url += `&key_id=${encodeURIComponent(keyId)}`;
                    if (contentKey)
                        url += `&content_key=${encodeURIComponent(contentKey)}`;
                    tt.navigateTo({ url });
                }
                else {
    ...
    unbindDevice() {
        tt.showModal({
            ...
            success: (res) => {
                if (res.confirm) {
                    const deviceId = this.data.deviceId || (0, storage_1.getDeviceId)();
                    const token = (0, storage_1.getUserToken)();
                    const clientToken = (0, storage_1.getClientToken)();
                    if (!deviceId || !token) {
                        tt.showToast({ title: '未登录', icon: 'none' });
                        return;
                    }
                    const base = (0, storage_1.getServerUrl)();
    ...
    async submitRedeem() {
        ...
        try {
            const r = await (0, subscription_1.redeemCode)(code);
            tt.showToast({ title: `已激活 ${r.plan}`, icon: 'success' });
            this.setData({ redeemInput: '' });
            this.refreshSubscription();
        }
        catch (err) {
            const msg = err?.error === 'invalid_format' ? '兑换码格式不正确' :
