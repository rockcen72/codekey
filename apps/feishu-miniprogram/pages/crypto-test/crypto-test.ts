/**
 * Crypto POC ‚Ä?validates the formal utils/crypto.ts module in WeChat runtime.
 * No self-contained code; everything comes from the production crypto module.
 */

// ‚îÄ‚îÄ Load formal crypto module (polyfill + vendor require baked in) ‚îÄ
  ...
  async onLoad() {
    ...
    if (typeof cryptoModule.encrypt === 'function') {
      ...
      try {
        ...
        const pt = 'Generated key works! üåç';
        const sealed = await encrypt(pt, kp.keyBytes, aad);
        const decrypted = await decrypt(sealed, kp.keyBytes, aad);
        rec('GenerateContentKey ‚Ü?roundtrip', decrypted === pt, 'sealed len=' + sealed.length + ', ok');
        dbg.push('gen-key=' + kp.keyHex.slice(0,8));
      } catch (e: any) { rec('GenerateContentKey ‚Ü?roundtrip', false, (e.message||'').slice(0,80)); }
    }

    // Test 3: tt.getRandomValues structure
    try {
      const raw: any = await tt.getRandomValues({ length: 32 });
