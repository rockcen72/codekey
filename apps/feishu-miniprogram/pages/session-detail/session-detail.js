...
const decryptedEventCache = new Map();
const decryptionFailureLogged = new Set();
const localCommandCacheBySession = new Map();
let fetchDetailSeq = 0;
function localCommandStorageKey(sessionId) {
    return `codekey_local_commands_${sessionId}`;
}
function loadLocalCommandMessages(sessionId) {
    if (!sessionId)
        return [];
    ...
    try {
        ...
        if (Array.isArray(stored)) {
            const messages = stored
                .filter((item) => item && typeof item.id === 'string' && typeof item.content === 'string' && typeof item.createdAt === 'string')
                .slice(-50);
...
function saveLocalCommandMessages(sessionId, messages) {
    if (!sessionId)
        return;
 ...
 */
async function decryptRawEvents(events) {
    const contentKey = (0, storage_1.getContentKey)();
    const deviceId = (0, storage_1.getDeviceId)();
    // === DIAGNOSTIC ===
    ...
    console.log('[decryptRawEvents] storedKeyId:', (0, storage_1.getKeyId)());
    ...
    if (!contentKey || !deviceId)
        return events;
    const storedKeyId = (0, storage_1.getKeyId)();
    const out = events.map((event) => ({ ...event }));
    const tasks = [];
    for (const event of out) {
        if (!event.sealed_payload || !event.key_id)
            continue;
        ...
        const aadFields = {
            ...
            eventId: data.clientEventId || event.id,
            eventType: event.type,
        };
        tasks.push((0, crypto_1.decryptEventPayload)(sealed, data, contentKey, aadFields)
            .then((merged) => {
...
function getEncryptedPlaceholder(data) {
    if (data?.e2eKeyStale === true)
        return '加密内容不可用（密钥已更新，请重新配对手机）';
    if (data?.encryption_error === true)
        return '加密内容不可用（桌面端加密失败）';
...
Page({
    ...
    subscribeWs() {
        ...
        this._onSessionLabelUpdatedBound = (payload) => {
            if (payload.sessionId === this.data.sessionId) {
        ...
        this._onEventResolvedBound = (payload) => {
            if (payload.sessionId !== this.data.sessionId)
                return;
            ...
            if (eventId) {
                ...
                const idx = messages.findIndex((m) => m.eventId === eventId && m.pending);
                if (idx !== -1) {
    ...
    unsubscribeWs() {
        if (this._onEventResolvedBound)
            app.offWsEvent('event_resolved', this._onEventResolvedBound);
        if (this._onEventPushBound)
            app.offWsEvent('event_push', this._onEventPushBound);
        if (this._onSessionDeactivatedBound)
            app.offWsEvent('session_deactivated', this._onSessionDeactivatedBound);
        ...
        if (this._onWsErrorBound)
            app.offWsEvent('error', this._onWsErrorBound);
        if (this._onSessionLabelUpdatedBound)
            app.offWsEvent('session_label_updated', this._onSessionLabelUpdatedBound);
    ...
    _startPolling() {
        this._stopPolling();
        this._pollTimer = setInterval(() => this.fetchDetail(), 10000);
    },
    _stopPolling() {
        if (this._pollTimer) {
    ...
    async fetchDetail(options) {
        ...
        try {
            ...
            {
                const ck = (0, storage_1.getContentKey)();
                let status;
                let latestSealed;
                if (!ck) {
                    status = 'disabled';
                ...
                if (status === 'stale') {
                    (0, storage_1.setE2EState)({ state: 'stale', localKeyId: (0, storage_1.getKeyId)(), lastServerKeyId: latestSealed?.key_id ?? null });
                }
                else {
                    (0, storage_1.setE2EStatus)(status);
                }
            }
            ...
            const freshEventMap = new Map();
            for (const e of rawEvents)
                freshEventMap.set(e.id, e);
            const stalePending = this.data.chatMessages.filter((m) => m.pending && m.type === 'ai' && m.eventId);
    ...
    buildChatMessages(rawEvents, scrollToEventId) {
        ...
        for (const message of cachedLocalCommands)
            localCommandMap.set(message.id, message);
        for (const message of (this.data.localCommandMessages || []))
            localCommandMap.set(message.id, message);
        ...
        let pendingCommandStarted = null;
        const flushPendingCommandStarted = () => {
            if (!pendingCommandStarted)
                return;
        ...
        for (const e of sorted) {
            ...
            if (effectiveType === 'user_prompt') {
                ...
                if (prompt === lastUserPrompt && e.data?.localOnly !== true)
                    continue;
            ...
            if (e.type === 'approval_required') {
                ...
                const riskText = RISK_LABELS[e.risk_level] || '未知';
                const accent = e.pending
                    ? 'pending'
                    : e.decision === 'approve'
                        ? 'approved'
                        : e.decision === 'deny'
            ...
            if (e.type === 'input_required') {
                ...
                const accent = e.pending ? 'pending' : 'neutral';
                messages.push({
                    id: e.id,
                    type: 'ai',
                    side: 'left',
    ...
    appendMissingLocalCommands(messages) {
        const cachedLocalCommands = loadLocalCommandMessages(this.data.sessionId);
    ...
    getPrimaryPendingEvent(messages) {
        for (let i = messages.length - 1; i >= 0; i--) {
            const msg = messages[i];
            if (msg.type === 'ai' && msg.pending)
                return msg;
        }
        return null;
    },
    ...
    openApprovalSheet(e) {
        ...
        const approvalEvent = this.data.chatMessages.find((m) => m.eventId === eventId && m.type === 'ai') || null;
    ...
    sendSheetReply() {
        ...
        if (!text || !this.data.approvalEvent)
            return;
        ...
        const messages = [...this.data.chatMessages];
        const aiIdx = messages.findIndex((m) => m.eventId === eventId && m.type === 'ai');
        if (aiIdx !== -1) {
    ...
    sendDockReply() {
        ...
        if (!text || !this.data.primaryPendingEvent)
            return;
    ...
    sendDecision(eventId, decision) {
        ...
        const dupIdx = messages.findIndex((m) => m.eventId === eventId + '-decision');
        if (dupIdx === -1) {
            messages.push({
                id: eventId + '-decision',
                type: 'user',
    ...
    async sendCommand() {
        ...
        const contentKeyHex = (0, storage_1.getContentKey)();
        const keyId = (0, storage_1.getKeyId)();
        const deviceId = (0, storage_1.getDeviceId)();
        const sentAt = new Date().toISOString();
        const commandId = (0, crypto_1.generateUUID)();
        const localCommandId = `local-command-${commandId}`;
        let payload;
        if (contentKeyHex && keyId && deviceId) {
            const e2eState = (0, storage_1.getE2EState)();
            if (e2eState.state === 'stale') {
                const sameSession = e2eState.lastToastSessionId === this.data.sessionId;
                if (sameSession && Date.now() - e2eState.lastToastAt < 30000) {
                    return;
                }
                (0, storage_1.setE2EState)({ state: 'stale', localKeyId: keyId, lastToastAt: Date.now(), lastToastSessionId: this.data.sessionId });
                tt.showToast({ title: 'E2E 密钥已过期，请在电脑上重新配对', icon: 'none' });
                return;
            }
            try {
                const envelope = await (0, crypto_1.encryptCommandPayload)(text, contentKeyHex, keyId, deviceId, this.data.sessionId, commandId);
                payload = {
    ...
    formatInputContent(data) {
        ...
        for (const q of questions) {
            ...
            if (text && !lines.includes(String(text)))
...
function agentChatName(agentType) {
    if (!agentType)
        return 'agent';
